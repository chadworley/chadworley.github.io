...
Finding the [reference angle](https://courses.lumenlearning.com/csn-precalculus/chapter/reference-angles/) by using the arctan function with the quotient of the absolute values of the rectangular coordinates.
...
Unfortunately, you cannot simply use $\arctan\left(\frac{`r ya`}{`r xa`}\right)$ to get the angle $\theta$. This is because tangent is not a one-to-one function, so arctangent has a restricted range, from $\frac{-\pi}{2}$ to $\frac{\pi}{2}$.
