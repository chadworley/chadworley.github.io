...
In Calculus, when [integrating](https://en.wikipedia.org/wiki/Integral) [rational expressions](https://en.wikipedia.org/wiki/Algebraic_fraction#Rational_fractions), it is often helpful to perfom [partial-fraction decomposition](https://openstax.org/books/precalculus-2e/pages/9-4-partial-fractions). The simplest version of partial-fraction decomposition occurs when there are two distinct linear factors.

The procedure can be exemplified by trying to determine $A$ and $B$ to satisfy the equation below **for all** values of $x$ (with restrictions $x\ne `r -a`$ and $x\ne `r -b`$).
