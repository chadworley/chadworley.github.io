...
In Calculus, when [integrating](https://en.wikipedia.org/wiki/Integral) [rational expressions](https://en.wikipedia.org/wiki/Algebraic_fraction#Rational_fractions), it is often helpful to perfom [partial-fraction decomposition](https://openstax.org/books/precalculus-2e/pages/9-4-partial-fractions). One version of partial-fraction decomposition occurs when there is a squared linear factor.

The procedure can be exemplified by trying to determine $A$ and $B$ to satisfy the equation below **for all** values of $x$ (with restriction $x\ne `r -a`$). It should be said that knowing/remembering how to set up the equation below is the hardest part of partial-fraction decomposition.
