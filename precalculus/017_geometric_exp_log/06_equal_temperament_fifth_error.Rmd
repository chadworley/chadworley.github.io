...
First, determine the [cents]((https://en.wikipedia.org/wiki/Cent_(music))) of an exact 3:2 ratio.

$$I_\text{exact} ~=~ 1200\cdot\log_2(1.5) ~=~ `r 1200*log(1.5,2)`$$

Determine the best [fret](https://en.wikipedia.org/wiki/Fret) by [rounding](https://en.wikipedia.org/wiki/Rounding#Rounding_to_the_nearest_integer) a logarithmic function.

$$x ~=~ \text{round} \left(`r N`\cdot \log_2(1.5) \right)$$
Then, determine the ratio of frequencies between an open string and the $x$th fret. It should be near 1.5, but a bit off.
...
A ratio of `r R` is equivalent to `r Ix` cents. We can get the final answer by taking the absolute difference.
