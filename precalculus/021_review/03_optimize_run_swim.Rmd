...
A lifeguard wants to get to a struggling swimmer as soon as possible. The lifeguard can run along the beach at `r ls` m/s and swim at `r ss` m/s. The lifeguard will run $x$ meters and swim $\sqrt{(`r hd`-x)^2+`r vd`^2}$ meters.
...
$$t(x) ~=~ \frac{x}{`r ls`} + \frac{\sqrt{(`r hd`-x)^2+`r vd`^2}}{`r ss`}$$
