...
2. In B1 type `N`
3. In B2 type `=count(A:A)`
4. In B3 type `mu`
5. In B4 type `=average(A:A)`
6. In B5 type `sigma`
7. In B6 type `=stdevp(A:A)`
8. In D1 type `=index($A:$A,RANDBETWEEN(1,$B$2),1)`, and drag formula down to D100
9. In A101 type `=1`
...
1. Paste the chart into the Doc
