...
//Global Variables:
float k = 0.047; //changing k and f is one way to get new behavior
float f = 0.02;
int rnds = 8; //rounds of cell quadruplication
int spr = 1000; //steps per round
int Sxi = 75; //initial size in x direction (Size_(x,initial))
int Syi = 45; //initial size in y direction
float h = 1.0; //initial distance between two cell's centers
float DA = 0.5; //diffusivity of A
float DB = 0.25; //diffusivity of B
float dt = 0.3; //the change in time for each step
float cl = 0.0001; //chaos level
string aaa = "zz/zw"; //output-file tag
...
int main()
{
  ...
  //initialize such that A=1 and B=0 everywhere except the center
	...
	A[Sy/2+Sy/2*Sx] = 0.0;
	B[Sy/2+Sy/2*Sx] = 1.0;
	A[Sx-Sy/2+Sy/2*Sx] = 0.0;
	B[Sx-Sy/2+Sy/2*Sx] = 1.0;
