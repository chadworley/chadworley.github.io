...
int Sxi = 102; //initial size in x direction (Size_(x,initial))
int Syi = 66; //initial size in y direction
float h = 1.0; //initial distance between two cell's centers
float DA = 0.5; //diffusivity of A
float DB = 0.25; //diffusivity of B
float dt = 0.4; //the change in time for each step
...
int main()
{
  ...
  //initialize such that A=1 and B=0 everywhere except the center
	...
	A[Sx/3+Sy/2*Sx] = 0.0;
	B[Sx/3+Sy/2*Sx] = 1.0;
	A[Sx-Sx/3+Sy/2*Sx] = 0.0;
	B[Sx-Sx/3+Sy/2*Sx] = 1.0;
