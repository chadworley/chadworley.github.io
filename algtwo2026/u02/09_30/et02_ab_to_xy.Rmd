...
The transformations are a translation `r lr` by `r abs(h)` and a translation `r du` by `r abs(k)`. This means we will `r hsa` `r abs(h)` `r hft` each $a$ value to find each $x$ value. We will also `r vsa` `r abs(k)` `r vft` each $b$ value to find each $y$ value.
