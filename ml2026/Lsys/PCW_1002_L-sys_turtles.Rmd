...
# PCW_1002: Lindenmayer Systems and Turtle Graphics
...
A [Lindenmayer system](https://en.wikipedia.org/wiki/L-system) (or L-system) is a set of rewrite rules. When these rules are repeatedly applied to a starting string ("word"), we can get complex results from simple rules. 
...
s = "xx[+FFF-FFxx+F-FF-FF-F[-F+F]F-F][-FFF+FFxx-F+FF+FF+F[+F-F]F+F]xx[+F-F][-F+F]"
s1 = substr(s,1,50)
s2 = substr(s,51,nchar(s))
...
plot(0,0,"n",axes=F,ann=F,xlim=c(-8,8),ylim=c(-3,11))
abline(h=0,col="grey")
abline(v=0,col="grey")
for(i in -10:10){
    for(j in -5:15){
