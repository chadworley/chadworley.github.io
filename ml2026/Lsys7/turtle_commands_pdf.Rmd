...
## ET_1021 version `r params$rseed`
...
```
\
* Number of recursions (number of iterations): `r goodies[[gc]][[3]]`
* Angle: `r goodies[[gc]][[4]]`°

Use the following sites to generate the pattern: 

* [https://chadworley.github.io/Lsys_string_gen.html](https://chadworley.github.io/Lsys_string_gen.html)
* [https://scratch.mit.edu/projects/1225522722/editor/](https://scratch.mit.edu/projects/1225522722/editor/)


```{r,echo=F,fig.dim=c(5,5),fig.align='center'}
par(mfrow=c(3,3),pty="s",mar=c(1,1.5,2,1.5))
jj = 0
for(i in gs){
    jj = jj+1
    ...
    makeplot(xyc,lwd=0.5)
    mtext(LETTERS[jj],3)
}
```
\

Which pattern is created?


```{r,echo=F,results='asis'}
if(params$showsol){
    cat("\\color{red}\n\n")
    cat(LETTERS[which(gs==gc)])
    cat("\n\n\\color{black}\n")
}
```
