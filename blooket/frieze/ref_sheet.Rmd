...
getbasedots = function(fg=1,n=4){
    if(fg<6){
        ...
        y = runif(2*n)-0.5
        h = runif(2*n)
        p = sample(c(0,1,3,4,5,7,8,9,10,11,12,13,15,16,18,19,20),2*n)
    } else {
        x = runif(n)
        y = runif(n)-0.5
        h = runif(n)
        p = sample(c(0,1,3,4,5,7,8,9,10,11,12,13,15,16,18,19,20),n)
...
par(mar=c(1,1,2,1),mfrow=c(7,2))


for(i in 1:7){
    set.seed(3)
    dots = getbasedots(i,3)
    plot(0,0,type="n",xlim=c(2.2,7.8),ylim=c(-0.7,0.7),axes=F,ann=F)
    drawdots(dots,i,0.7,2,2)
    mtext(paste0(w1[i]," (",w2[i],")"),3)
...
n2 = c("hop (translations)","step (glide reflections)","sidle (mirror reflections)","spinning hop (180° rotations)","spinning sidle","jump","spinning jump")
