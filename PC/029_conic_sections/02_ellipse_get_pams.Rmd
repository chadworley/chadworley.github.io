...
for(i in 1:length(ord)){
  ...
  r1z = r1s[j]
  r2z = r2s[j]
  exp1 = paste0("\\frac{(x-",hz,")^2}{",r1z^2,"}+\\frac{(y-",kz,")^2}{",r2z^2,"}=1",collapse="")
  exp1 = fixer(exp1)
  EE = c(EE,exp1)
}
ace = LETTERS[1:16]
...
An [ellipse](https://en.wikipedia.org/wiki/Ellipse) is the set of ($x$, $y$) points that satisfy the equation below.
...
$$\begin{align}
A&:~~`r EE[1]` & B&:~~`r EE[2]`  & C&:~~`r EE[3]`  & D&:~~`r EE[4]` \\
E&:~~`r EE[5]` & F&:~~`r EE[6]`  & G&:~~`r EE[7]`  & H&:~~`r EE[8]` \\
I&:~~`r EE[9]` & J&:~~`r EE[10]`  & K&:~~`r EE[11]`  & L&:~~`r EE[12]` \\
M&:~~`r EE[13]` & N&:~~`r EE[14]`  & O&:~~`r EE[15]`  & P&:~~`r EE[16]` \\
\end{align}$$
