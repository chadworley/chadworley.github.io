...
A right triangle has an angle of `r angt` radians. The adjacent leg has a length of `r adjt` meters. How long, in meters, is the leg opposite the angle?
...
$$\tan(`r angt`) = \frac{x}{`r adjt`}$$
Multiply both sides by `r adjt` to solve for $x$.

$$`r adjt` \cdot \tan(`r angt`) = x$$
To evaluate $x$, type the following into a modern calculator: ``round(`r adjt`*tan(`r angt`))``. Make sure to be in radian mode, not degree mode.
