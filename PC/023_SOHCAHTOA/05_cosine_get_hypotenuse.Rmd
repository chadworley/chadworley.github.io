...
A right triangle has an angle of `r angt` radians. The adjacent leg has a length of `r adjt` meters. How long, in meters, is the hypotenuse?
...
$$\cos(`r angt`) = \frac{`r adjt`}{x}$$
Multiply both sides by $x$ and divide both sides by $\cos(`r angt`)$ to solve for $x$.

$$x = \frac{`r adjt`}{\cos(`r angt`)}$$
To evaluate $x$, type the following into a modern calculator: ``round(`r adjt`/cos(`r angt`))``. Make sure to be in radian mode, not degree mode.
