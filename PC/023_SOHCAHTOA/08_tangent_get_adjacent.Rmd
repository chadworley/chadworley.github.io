...
A right triangle has an angle of `r angt` radians. The opposite leg has a length of `r oppt` meters. How long, in meters, is the leg adjacent the angle?
...
$$\tan(`r angt`) = \frac{`r oppt`}{x}$$
Multiply both sides by $x$, and divide both sides by $\tan(`r ang`)$ to solve for $x$.

$$x = \frac{`r oppt`}{\tan(`r angt`)}$$
To evaluate $x$, type the following into a modern calculator: ``round(`r oppt`/tan(`r angt`))``. Make sure to be in radian mode, not degree mode.
