...
A right triangle has an angle of `r angt` radians. The hypotenuse has a length of `r hypt` meters. How long, in meters, is the leg adjacent the angle?
...
$$\cos(`r angt`) = \frac{x}{`r hypt`}$$
Multiply both sides by `r hypt` to solve for $x$.

$$`r hypt` \cdot \cos(`r angt`) = x$$
To evaluate $x$, type the following into a modern calculator: ``round(`r hypt`*cos(`r angt`))``. Make sure to be in radian mode, not degree mode.
