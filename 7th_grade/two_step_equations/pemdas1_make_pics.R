library(latex2exp)

fixer = function(ex){
  ex = gsub("")
}

s=""


for(i in 1:100){
    print(i)
    qt = sample(1:8,1)
    if(qt==1){ #(x+a)*b
        while(T){
            xa = sample(1:9,2,T)
            if(sum(xa)<=10){
                break
            }
        }
        x = xa[1]
        a = xa[2]
        b = sample(2:10,1)
        c = (x+a)*b
        ex1 = paste0("(",x,"+",a,")\\cdot",b)
        ex2 = paste0(b,"\\cdot (",x,"+",a,")")
        ex3 = paste0(b," (",x,"+",a,")")
        exes = c(ex1,ex2,ex3)
        ex = sample(exes,1)
        sol = c
    }
    if(qt==2){ #(x-a)*b
        while(T){
            xa = sample(1:12,2)
            x = xa[1]
            a = xa[2]
            if(x-a>0 && x-a<11){
                break
            }
        }
        b = sample(2:10,1)
        c = (x-a)*b
        ex1 = paste0("(",x,"-",a,")\\cdot",b)
        ex2 = paste0(b,"\\cdot (",x,"-",a,")")
        ex3 = paste0(b," (",x,"-",a,")")
        exes = c(ex1,ex2,ex3)
        ex = sample(exes,1)
        sol = c
    }
    if(qt==3){ #(x+a)/b
        while(T){
            c = sample(1:10,1)
            b = sample(2:10,1)
            if(sample(1:2,1)==1){
                x = sample(1:10,1)
                a = c*b-x
            } else {
                a = sample(1:10,1)
                x = c*b-a
            }
            if(x>0 && a>0){break}
        }
        ex1 = paste0("\\frac{",x,"+",a,"}{",b,"}")
        exes = ex1
        ex = sample(exes,1)
        sol = c
    }
    if(qt==4){ #(x-a)/b
        while(T){
            c = sample(1:10,1)
            b = sample(2:10,1)
            if(sample(1:2,1)==1){
                x = sample(1:10,1)
                a = x-c*b
            } else {
                a = sample(1:10,1)
                x = c*b+a
            }
            if(x>0 && a>0){break}
        }
        ex1 = paste0("\\frac{",x,"-",a,"}{",b,"}")
        exes = ex1
        ex = sample(exes,1)
        sol = c
    }
    if(qt==5){ #ax+b
        a = sample(2:10,1)
        x = sample(1:10,1)
        b = sample(1:10,1)
        c = a*x+b
        ex1 = paste0(a,"\\cdot ",x,"+",b)
        ex2 = paste0(b,"+",a,"\\cdot",x)
        exes = c(ex1,ex2)
        ex = sample(exes,1)
        sol = c
    }
    if(qt==6){ #ax-b
        while(T){
            a = sample(2:10,1)
            x = sample(1:10,1)
            b = sample(1:10,1)
            c = a*x-b
            if(c>0){break}
        }
        ex1 = paste0(a,"\\cdot ",x,"-",b)
        exes = c(ex1)
        ex = sample(exes,1)
        sol = c
    }
    if(qt==7){ #x/a+b
        cmb = sample(2:10,1)
        b = sample(1:9,1)
        c = cmb+b
        a = sample(2:10,1)
        x = (c-b)*a
        ex1 = paste0("\\frac{",x,"}{",a,"}+",b)
        ex2 = paste0(b,"+\\frac{",x,"}{",a,"}")
        exes = c(ex1,ex2)
        ex = sample(exes,1)
        sol = c
    }
    if(qt==8){ #x/a-b
        cpb = sample(2:10,1)
        b = sample(1:(cpb-1),1)
        c = cpb-b
        a = sample(2:10,1)
        x = (c+b)*a
        ex1 = paste0("\\frac{",x,"}{",a,"}-",b)
        exes = c(ex1)
        ex = sample(exes,1)
        sol = c
    }
    s2 = "\\documentclass[border=5pt]{standalone}
        \\begin{document}
        $myex$
        \\end{document}"
    s2 = gsub("myex",ex,s2,fixed=T)
    fileConn<-file("tmp.tex")
    writeLines(s2, fileConn)
    close(fileConn)
    tinytex::xelatex("tmp.tex")
    cmand = "magick -density 600 tmp.pdf -background white -flatten FOPNG"
    ofn = paste0("pemdas1/q",sprintf("%03d",i),".png")
    cmand = gsub("FOPNG",ofn,cmand,fixed=T)
    system(cmand)
    s = paste0(s,sol,"\n",collapse="")
}


fileConn<-file("pemdas1/sols.txt")
writeLines(s, fileConn)
