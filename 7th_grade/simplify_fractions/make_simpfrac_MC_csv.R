...
for(i in 1:1000){
    ...
    pf2 = paste0(f2[1],"/",f2[2],collapse="")
    pf3 = paste0(f3[1],"/",f3[2],collapse="")
    pf4 = paste0(f4[1],"/",f4[2],collapse="")
    pfs = c(pf1,pf2,pf3,pf4)
    ord = sample(1:4)
    psh = pfs[ord]
    ans = which(ord==1)
    qstr = paste0("Simplify the fraction ",pfq,".",collapse="")
    q = paste0(i,',"',qstr,'",',psh[1],",",psh[2],",",psh[3],",",psh[4],",12,",ans,",,",collapse="")
