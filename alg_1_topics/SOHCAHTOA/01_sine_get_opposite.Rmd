...
A right triangle has an angle of `r angd` degrees. The hypotenuse has a length of `r hypt` meters. How long, in meters, is the leg opposite the angle?
...
$$\sin(`r angd`^\circ) = \frac{x}{`r hypt`}$$
Multiply both sides by `r hypt` to solve for $x$.

$$`r hypt` \cdot \sin(`r angd`^\circ) = x$$
To evaluate $x$, type the following into a modern calculator: ``round(`r hypt`*sin(`r angd`))``. Make sure to be in degree mode, not radian mode.
