...
while(T){
  ...
  if(opp2!=adj2 && hyp-opp2>2 && hyp-adj2>2 && opp2>10 && adj>10 && round(ang*adj2)!=opp2 && round(ang*opp2)!=adj2){break}
}
```
...
A right triangle has an angle of `r angd` degrees. The opposite leg has a length of `r oppt` meters. How long, in meters, is the leg adjacent the angle?
...
text(cos(ang)/2,-0.05,"? m",col="blue",cex=0.8)
...
In this case, we are given the angle and the **opposite**, and we are asked for the **adjacent**. The trigonometric ratio that relates these two sides is [tangent](https://en.wikipedia.org/wiki/Trigonometric_functions). Let $x$ represent the requested length.

$$\tan(`r angd`^\circ) = \frac{`r oppt`}{x}$$
Multiply both sides by $x$, and divide both sides by $\tan(`r angd`)$ to solve for $x$.

$$x = \frac{`r oppt`}{\tan(`r angd`)}$$
To evaluate $x$, type the following into a modern calculator: ``round(`r oppt`/tan(`r angd`))``. Make sure to be in degree mode, not radian mode.
