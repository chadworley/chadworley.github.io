...
In general, if point $(x,y)$ is rotated by $\theta$ radians (and clockwise is negative while counterclockwise is positive), the new point $(x',y')$ can be determined with the [following equations](https://en.wikipedia.org/wiki/Rotation_(mathematics)#Two_dimensions):
...
$$x' ~=~ `r X1` \cos(`r th`) - (`r Y1`)\sin(`r th`) ~\approx~ `r X2`$$
$$y' ~=~ `r X1` \sin(`r th`) + (`r Y1`)\cos(`r th`)  ~\approx~ `r Y2`$$

So, the answer is `r sol`.
