...
while(T){
    ...
    ol = round(runif(1,10,100),1)
    hl = round(ol/sin(th),1)
    al = hl*cos(th)
    ans = asin(ol/hl)
    tol = 0.01
    an2 = signif(ans,3)
    ...
    if(ol>10 && al>10 && hl<100 && canpass){break}
}

angstr = "x"
ostr = sprintf("%.1f",signif(ol,3))
astr = ""
hstr = sprintf("%.1f",signif(hl,3))

if(th<1){
    angstr2 = sprintf("%.3f",signif(ans,3))
    tol = 0.001
} else {
    angstr2 = sprintf("%.2f",signif(ans,3))
    tol = 0.01
}
...
xlang = bmp*cos(th/2)/th+1*bmp*cos(th/2)
ylang = bmp*sin(th/2)/th+1*bmp*sin(th/2)
srtang = th/2/pi*180
...
$$\sin(x)~=~\frac{`r ol`}{`r hl`}$$

So,


$$x~=~\arcsin\left(\frac{`r ol`}{`r hl`}\right)$$
