...
while(T){
    ...
    tht = atan2(yt,xt)
    degt = tht*180/pi
    if(abs(diff(c(vr,vc)))>0.2 && abs(deg-90)>5 && abs(degt-deg)>2 && 
       abs(rt-max(c(vr,vc)))>0.1 && round(rt,1)!=round(vc,1)){break}
}

ans1 = round(rt,1)
ans2 = round(degt)
ach1 = ans1+(0:5-sample(0:5,1))*0.1
ach2 = ans2+(0:5-sample(0:5,1))*1
sol1 = paste0(1*(ach1==ans1),collapse="")
sol2 = paste0(1*(ach2==ans2),collapse="")
al1 = paste0("* ",sprintf("%.01f m/s",ach1),collapse="\n")
al2 = paste0("* ",sprintf("%.0f°",ach2),collapse="\n")
...
A canoe travels `r sprintf("%.01f",vc)` m/s **relative to the water**. A river, heading east, flows at `r sprintf("%.01f",vr)` m/s. The canoe points at a direction `r sprintf("%.0f",deg)`° counterclockwise from east.
...
text(-0.1+0.15*cos(th/2),-0.5+0.15*sin(th/2),paste0(sprintf("%.0f",round(deg)),"°"),srt=(deg/2+90)%%180-90,adj=c(0.5-0.7*sign(cos(th/2)),0.5),col="red")
...
Find the canoe's total speed, in m/s, and direction of travel, as degrees counterclockwise from east. (In other words, find the canoe's velocity **relative to the ground**, in polar form.)
...
We will use $\vec{v}_1$ for the canoe's velocity **relative to water**. It is shown below with approximate rectangular coordinates (found from formulas above, using $r=`r vc`$ and $\theta=`r deg`$°).
...
$$\theta=\arctan(`r sprintf("%.03f",yt)`,`r sprintf("%.03f",xt)`)~=~`r degt`°$$
...
So, if we round like the answer choices, the speed of the canoe is `r sprintf("%.01f",rt)` m/s at a direction of `r sprintf("%.0f",degt)`° counterclockwise from east.
