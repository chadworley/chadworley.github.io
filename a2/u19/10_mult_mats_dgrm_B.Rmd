...
while(T){
    P = matrix(sample(1:9,nf*ns,F),ncol=nf)
    Q = matrix(sample(1:9,nf*np,F),nrow=nf)
    C = P%*%Q
    unkij = sample(1:3,2)
    unki = unkij[1]
    unkj = unkij[2]
...
A third matrix, $C$, is equal to the [product](https://en.wikipedia.org/wiki/Matrix_multiplication) $AB$. A student set up a diagram to help find the product:
...
lines(c(np+0.5,np+0.5),c(nf,0)+0.5,lwd=3)
lines(c(np+0.5,np+ns+0.5),c(nf,nf)+0.5,lwd=3)
...
If you repeat this process for all 9 elements in $C$, you get:
