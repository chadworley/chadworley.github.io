...
horv = sample(1:2,1)
k = round(runif(1,0.4,0.666)*sample(c(-1,1),1),3)
if(horv==1){
    t = matrix(c(1,k,0,1),2)
    tstr = matrix(c("1","k","0","1"),2)
    wrd = "horizontal"
    xory = 1
} else {
    t = matrix(c(1,0,k,1),2)
    tstr = matrix(c("1","0","k","1"),2)
    wrd = "vertical"
    xory = 2
}
...
A video-game developer is hoping to apply a `r wrd` [shear](https://en.wikipedia.org/wiki/Shear_mapping) to a set of points by using matrix multiplication.
...
for(i in 1:n){
    xx = seq(x1[i],f[i,1],length.out=100)
    yy = seq(y1[i],f[i,2],length.out=100)
    for(j in 1:100){
        points(xx[j],yy[j],col=hsv(hues[i],0.1+0.8*j/100,1),cex=0.03)
    }
}
...
The `r wrd` shear matrix can be found by using $k=`r k`$ in the formula matrix.
...
What is the new `r c("x","y")[xory]` coordinate when point $(`r x1[wunk]`, `r y1[wunk]`)$ undergoes the `r wrd` shear transformation with $k=`r k`$?

Solution
========

The `r wrd` shear transformation matrix:

$$T=`r matpr(t)`$$

We are looking for the new `r c("x","y")[xory]` coordinate after point $(`r x1[wunk]`, `r y1[wunk]`)$ undergoes the shear transformation. Use column `r xory` of $T$:
