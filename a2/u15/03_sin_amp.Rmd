...
In general, for periodic functions, if we find **half the difference** of the max minus min, we call that the **[amplitude](https://en.wikipedia.org/wiki/Amplitude)**.
...
Also, some waves are spatial (instead of temporal). In that case, the distance between peaks is the [wavelength](https://en.wikipedia.org/wiki/Wavelength) ($\lambda$). Spatio-temporal waves have both period and wavelength, which are related by the speed ($c$) of the wave. $c=\frac{\lambda}{P}$.
