...
exp0 = paste0("\\frac{",pfix(c(1,a+b,a*b),2:0),"}{",pfix(c(1,c+d,c*d),2:0),
              "}\\div \\frac{",pfix(c(1,a+f,a*f),2:0),"}{",pfix(c(1,c+e,c*e),2:0),"}")
...
Remember, dividing by a fraction is equivalent to multiplying by the reciprocal.

$$\frac{2}{3}\div \frac{5}{7} ~=~ \frac{2}{3}\cdot\frac{7}{5} ~=~ \frac{14}{15}$$

A similar technique is helpful for dividing rational expressions.
...
$$`r exp0` ~\equiv~ \frac{x^2+ax+b}{x^2+cx+d}$$
...
Start with the given quotient.

$$`r exp0`$$

Reciprocate the second rational fraction to convert the quotient into a product.
