...
# m/(x+p)-n/(x+q)
# (m*(x+q)-n(x+p))/(x^2+(p+q)x+pq)
# ((m-n)x+(mq-np))/(x^2+(p+q)x+pq)
...
exp4 = gsub("+{-","-{",exp4,fixed=T)
exp4 = gsub("{1}x","x",exp4,fixed=T)
exp4 = gsub("{-1}x","-x",exp4,fixed=T)

ans = c(m-n,m*q-n*p,p+q,p*q)
rans = c(sample(1:200,3),m+n,m*q+n*p,n*p-m*q,p-q,m+p,m+q,n+p,n+q,m*q-n*p,m*n)
...
Subtracting rational expressions is similar to subtracting fractions: specifically, a common denominator can be helpful when subtracting rational expressions. Find the parameters $a$, $b$, $c$, and $d$ such that the [identity](https://en.wikipedia.org/wiki/Identity_(mathematics)) below is equivalent for any value of $x$.
...
Now, since we have common denominators, we can subtract the numerators.
