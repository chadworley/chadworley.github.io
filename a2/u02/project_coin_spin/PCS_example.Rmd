...
We decided on a sample size of 200 spins. To run the simulation, we used a spreadsheet. Each fair spin was simulated with `=IF(RAND()<0.5,1,0)`. This command was used in an array of cells with 200 columns and 1000 rows. Each row is one repetition of the simulation. From those 1000 repetitions, we got 1000 proportions. As expected, because we simulated a fair coin, those proportions were near 0.5. We generated a histogram of those proportions.

```{r,echo=F}
set.seed(3)
x = rbinom(1000,200,0.5)/200
hist(x,main="1000 Repetitions of Spinning 200 Fair Coins",
     xlab="Proportion Heads",breaks=seq(0.35,0.65,0.02))
lb = sort(x)[25]
ub = sort(x)[975]
ME = (ub-lb)/2
```
