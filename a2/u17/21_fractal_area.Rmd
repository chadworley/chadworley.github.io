...
A fractal is generated such that the biggest circle has an area of 1 square units. The next generation has `r NN` circles, each with an area of $k=`r k^2`$ square units. In the next generation of `r NN^2` circles, each has an area of $k^2=`r k^4`$, then `r NN^3` circles with area $k^3$ each, etc...

Find the area of the fractal. The tolerance is $\pm 0.01$.

Solution
========

We find the common ratio $r = (`r NN`)(`r k^2`) = `r NN*k^2`$.

$$1 + `r rrr` + `r rrr`^2 + `r rrr`^3 + \ldots ~=~ \frac{1}{1-`r rrr`} ~=~ `r sol`$$
