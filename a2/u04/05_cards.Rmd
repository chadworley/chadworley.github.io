...
s1 = paste("$P(\\text{",is[i1],"})=\\frac{",sum(nums[i1,]),"}{",tot,"}=",p1,"$",sep="")
...
s2 = paste0("$P(\\text{",cs[c2],"})=\\frac{",sum(nums[,c2]),"}{",tot,"}=",p2,"$",sep="")
...
denom3 = paste(tot)
s3 = paste0("$P(\\text{",is[i3]," and ",cs[c3],"})=\\frac{",nums[i3,c3],"}{",tot,"}=",p3,"$",sep="")
...
numer4 = paste(paste(c(nums[i4,],nums[,c4]),collapse="+"),
               nums[i4,c4],sep="-")
denom4 = paste(tot)
s4 = paste0("$P(\\text{",is[i4]," or ",cs[c4],"})=\\frac{",sum(nums[i4,]),"+",sum(nums[,c4]),"-",nums[i4,c4],"}{",tot,"}=",p4,"$",sep="")
...
s5 = paste0("$P(\\text{",cs[c5]," given ",is[i5],"})=\\frac{",nums[i5,c5],"}{",sum(nums[i5,]),"}=",p5,"$",sep="")
...
s6 = paste0("$P(\\text{",is[i6]," given ",cs[c6],"})=\\frac{",nums[i6,c6],"}{",sum(nums[,c6]),"}=",p6,"$",sep="")
...
cho = lapply(cho,function(a){if(typeof(a)=="double"){sprintf("%.2f",a)}else{a}})
...
tt = ts[ord] #lapply(ts[ord],function(a){if(typeof(a)=="double"){sprintf("%.2f",a)}else{a}})
ss = sss[ord]
type = types[ord]
sol = sols[ord]
...
In a deck of strange cards, there are `r tot` cards. Each card has an image and a color. The number of repeats of each image-color combination is shown in the table below.
