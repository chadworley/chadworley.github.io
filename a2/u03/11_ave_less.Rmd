...
That spinner ($X$) will be fairly spun `r n` times, and the **sample mean** of the spins will be recorded. Determine the probability that the random sample mean is less than `r xa`.
...
[video](https://vimeo.com/531941126){target="_blank"}

We use the central limit formulas for a random mean.
$$\mu_{\bar{X}} = \mu_{X} = `r mu` $$
$$\sigma_{\bar{X}} = \frac{\sigma_{X}}{\sqrt{n}} = \frac{`r sig`}{\sqrt{`r n`}} = `r siga`$$
So, we think the random mean is normally distributed with a mean of `r mua` and a standard deviation of `r siga`.
