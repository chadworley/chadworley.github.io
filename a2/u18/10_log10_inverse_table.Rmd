...
If a parent function is $f(x)=10^x$ then the [inverse function](https://en.wikipedia.org/wiki/Inverse_function) is $f^{-1}(x)=\log_{10}(x)$. Use the 5 points given above to fill out the table below (with corresponding points in increasing order).

```{r tab4, results="asis", echo=F}
mydf4 = data.frame(paste0("##ANSWER",1:5,"##"),paste0("##ANSWER",6:10,"##"))
colnames(mydf4) = c("$x$","$y=\\log_{10}(x)$")
...
The inverse function $y=f^{-1}(x)=\log_{10}(x)$ is defined by the table below.

```{r tab3, results="asis", echo=F}
mydf3 = data.frame(paste0(f),paste0(x))
colnames(mydf3) = c("$x$","$y=\\log_{10}(x)$")
