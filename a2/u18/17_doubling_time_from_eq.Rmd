...
At $t=0$ a population is $y(0)=`r A0`$ million. The population grows with the following rate equation (assuming $\Delta t$ is very small):
$$\frac{\Delta y}{\Delta t} \approx `r k` y$$

where $t$ has units of years, $y$ has units of millions, and $k=`r k`$ has units of years$^{-1}$. The exact solution (using algebraic calculus instead of Euler's method) is an exponential function.
...
You could also use algebra to find $b$ so the following equation is true for all $t$.

$$`r A0`e^{`r k`t} = `r A0`(2)^{t/b} $$
