png("sc_cont_rounds.png",1000,1000)
...
for(i in (apr*rowbeg):(apr*rowend-1)){
  ang1 = i/apr*2*pi*lmult
  r1 = i/apr*rmult+radd
  x1 = r1*cos(ang1)
  y1 = r1*sin(ang1)
  ang2 = (i+1)/apr*2*pi*lmult
  r2 = (i+1)/apr*rmult+radd
  x2 = r2*cos(ang2)
  y2 = r2*sin(ang2)
  numdots = max(i%/%apr,0)+1
  ...
  for(j in 1:numdots){
    ...
    if(j==1){
      k = i+apr
      ang31 = k/apr*2*pi*lmult
      r31 = k/apr*rmult+radd
      x31 = r31*cos(ang31)
      y31 = r31*sin(ang31)
      ang32 = (k+1)/apr*2*pi*lmult
      r32 = (k+1)/apr*rmult+radd
      x32 = r32*cos(ang32)
      y32 = r32*sin(ang32)
      numdots33 = max(k%/%apr,0)+1
